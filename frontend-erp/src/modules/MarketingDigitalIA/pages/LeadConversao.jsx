...
function LeadConversao() {
  ...
  const salvar = async () => {
    ...
    fd.append('estagio', 'Convertido');
    fd.append('descricao', descricao);
    fd.append('cliente', JSON.stringify(form));
    files.forEach(f => fd.append('files', f));
    try {
      await fetchComAuth(`/leads/info/${id}`, {
        method: 'POST',
        body: fd,
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <label className="block md:col-span-2">
          <span className="text-sm">Descrição/Transcrição</span>
          <textarea className="input" rows="3" value={descricao} onChange={e => setDescricao(e.target.value)} />
          <input type="file" multiple className="mt-2" onChange={handleFileChange} />
        </label>
      </div>
