...
let globalId = parseInt(localStorage.getItem("globalPecaId")) || 1;

const Home = () => {
  const navigate = useNavigate();
  const [lotes, setLotes] = useState(JSON.parse(localStorage.getItem("lotes") || "[]"));
...
const Lote = () => {
  ...
  const pacotesComIds = novosPacotes.map(pacote => {
    const pecasComIds = pacote.pecas.map(p => {
      const id = globalId++;
      if (p.operacoes) {
        localStorage.setItem("op_" + id, JSON.stringify(p.operacoes));
      }
      return { ...p, id };
    });
  ...
  localStorage.setItem("globalPecaId", globalId);
  ...
  const gerarArquivos = async () => {
    const pecas = pacotes.flatMap(p => p.pecas.map(pc => ({
      ...pc,
      operacoes: JSON.parse(localStorage.getItem("op_" + pc.id) || "[]")
    })));
    const resposta = await fetch("http://127.0.0.1:8009/gerar-lote-final", {
      method: "POST",
        ...
        {pacotes.map((p, i) => (
          <li key={i} className="border p-2 rounded">
            <div className="flex justify-between">
              ...
              <div className="space-x-2">
                <Button onClick={() => navigate(`/lote/${nome}/pacote/${i}`)}>Editar</Button>
...
const App = () => (
  <Router>
    <Routes>
      ...
      <Route path="/lote/:nome" element={<Lote />} />
      <Route path="/lote/:nome/pacote/:indice" element={<Pacote />} />
      <Route path="/lote/:nome/peca/:peca" element={<EditarPeca />} />
    </Routes>
  </Router>
);
...
const EditarPeca = () => {
  ...
  useEffect(() => {
    ...
    if (pecaEncontrada) {
        ...
        const operacoesSalvas = JSON.parse(localStorage.getItem("op_" + pecaId) || "[]");
  ...
  const salvarOperacao = () => {
    ...
    if (operacao === "Puxador Cava") {
      ...
      let newPecaId = parseInt(localStorage.getItem("globalPecaId") || "1");
      localStorage.setItem("globalPecaId", newPecaId + 1);
      ...
      localStorage.setItem(`op_${newPecaId}`, JSON.stringify([]));
    ...
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <div className="border p-4 rounded bg-gray-50 overflow-x-auto w-full min-w-[620px]">
        ...
        <div className="flex gap-2 mt-4">
          ...
          <Button variant="outline" onClick={() => navigate(`/lote/${nome}`)}>Finalizar</Button>
